  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          className={cn(
            "h-7 min-w-7 shrink-0 flex flex-col items-stretch  justify-center  rounded-sm hover:bg-neutral-200/80 px-1.5 overflow-hidden text-sm"
          )}
        >
      ...
      <DropdownMenuContent className="p-0">
        {lineHeights.map(({ label, value }) => (
          <button
            key={value}
            onClick={() => {
              console.log("Setting line height to:", value);

              editor?.chain().focus().setLineHeight(value).run();
            }}
            className={cn(
              "flex items-center gap-x-2 px-2 py-1 rounded-sm w-full hover:bg-neutral-200/80",
...
const ImageButton = () => {
  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="h-7 min-w-7 shrink-0 flex flex-col items-center justify-center rounded-sm hover:bg-gray-200/80">
